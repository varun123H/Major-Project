...
def calculate_iou(true_mask, pred_mask):
    ...
    intercluster=np.sum(intersection)
    unicluster=np.sum(union)
    #print(intercluster)
    #print(unicluster)
    np.savetxt('intercluster.txt', intersection, fmt='%d')
    return iou
...
num_clusters = 10  # Adjust this value as needed
...
# Calculate the mean IOU and average pixel accuracy
...
plt.subplot(133)
plt.bar(range(num_clusters), iou_scores, tick_label=['Cluster {}'.format(i) for i in range(num_clusters)])
plt.ylim(0, 1)
plt.title('IOU Scores')

plt.figure(figsize=(8, 4))
plt.bar(range(num_clusters), pixel_accuracies, tick_label=['Cluster {}'.format(i) for i in range(num_clusters)])
plt.ylim(0, 100)
...
print("Average Pixel Accuracy: {:.2f}%".format(avg_pixel_accuracy))
